"""Capture DeliverStraw home preset from human demo episode 0."""
...
def main() -> None:
    dataset = get_ds_path("DeliverStraw", source="human", split="pretrain")
    if dataset is None:
        raise RuntimeError("DeliverStraw pretrain dataset not found")
    ...
    # Default home: step back from the drawer work area and raise the arm slightly.
    ...
    out = default_home_preset_path("DeliverStraw")
    assert out is not None
    out.parent.mkdir(parents=True, exist_ok=True)
    saved = save_home_preset(home, out)
    env.close()
    print(f"Saved DeliverStraw home preset to {saved}")
